...
/**
 ...
 action handler to your `KeyboardParentViewController`, by setting
...
//        triggerHapticFeedback(for: gesture, on: action)
    ...
    open func handleTap(on action: KeyboardAction, view: UIView) {
        ...
        if let gesturerecogniser = view.gestureRecognizers {
            for gesture in gesturerecogniser {
                if gesture.name == "Double-Tap-Gesture" {
                    handle(.repeatPress, on: action, view: view)
                    return
                }
            }
