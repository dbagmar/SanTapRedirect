...
class RegeneratePaymentURL{
    
    static func getRegenarateURL(genrateURLSErvice:ReGenerateURLServiceRequestModelRequest,skk:String,authoriztion:String,authToken:String,_ completion: @escaping ((NetworkResult<GenerateURLModelResponse, ErrorResult>) -> Void)){
        ...
        self.getRegenaratePaymentURL(data: getUrlServiceKey,RSA:skk,authoriztion: authoriztion, authToken: authToken){
            urlresult in
            switch urlresult{
    ...
    static func getRegenaratePaymentURL(data:AESEncryptionModel,RSA:String,authoriztion:String,authToken:String,_ completion: @escaping ((NetworkResult<AESDecryptionService, ErrorResult>) -> Void))
    {
        let entityUrl:String = MAIN_URL + "security/links/forwarding"
