//
//  SymmetricCryptorAlgorithm.swift
//  SantanderTap
//
...
class SymmetricCryptor: NSObject {
    ...
    class func randomStringOfLength(_ length:Int) -> String {
        var string = ""
        for _ in (1...length) {
            let str = kSymmetricCryptorRandomStringGeneratorCharset[Int(arc4random_uniform(UInt32(kSymmetricCryptorRandomStringGeneratorCharset.count) - 1))]
            string.append(str)
        }
        return string
    }
    ...
    // MARK: - Key based methods
    
    class func keyfLength(_ length:Int,initialString:String) -> String {
        var string = ""
        let charset: [Character] = initialString.map({$0})
        for i in (1...length) {
            let str = charset[i]
            string.append(str)
        }
    ...
    func setIVBasedOn(initialString:String) {
        let length = self.algorithm.requiredIVSize(self.options)
        self.iv = SymmetricCryptor.getIVData(length,initialString: initialString)
    }
    
    class func getIVData(_ length: Int,initialString:String) -> Data? {
        var string = ""
        var mutableData = Data(count: length)
        let charset: [Character] = initialString.map({$0})
        for i in(1...(length*2))
        ...
        mutableData.append(string.data(using: .utf8)!)
        return mutableData
    }
}
