//
//  ReGenerateURLServiceRequest.swift
//  Teclado
//
...
class PaymentCancelService{
    static func paymentCancelRequest(genrateURLSErvice:ReGenerateURLServiceRequestModelRequest,skk:String,authoriztion:String,authToken:String,_ completion: @escaping ((NetworkResult<PaymentCancelModelResponse, ErrorResult>) -> Void)){
        ...
        self.paymentCancel(data: getUrlServiceKey,RSA:skk,authoriztion: authoriztion, authToken: authToken){
            urlresult in
            switch urlresult{
            case .success(let aesData):
                let decodedString = AESEncryptionss.AesDecrypt(skkStr: skk, encodesString:aesData.key)
                                                
                if let decodedData = try? JSONDecoder().decode([PaymentCancelModelResponse].self, from: decodedString.data(using: .utf8)!)
                {
                    completion(.success(decodedData.first!))
                    break
                }else{
                    
                    completion(.failure(.parser(statusCode: 0, responseMessage: "payment data not found")))
                    break
                }
    ...
    static func paymentCancel(data:AESEncryptionModel,RSA:String,authoriztion:String,authToken:String,_ completion: @escaping ((NetworkResult<AESDecryptionService, ErrorResult>) -> Void))
    {
        
        let entityUrl:String = MAIN_URL + "security/links/canceled"
