...
open class KeyboardInputViewController: UIInputViewController {
    ...
    open override func viewDidLoad() {
        ...
        view.backgroundColor = Asset.Colors.grayButton.color
    ...
    public lazy var keyboardCenterStackView: UIStackView = {
        let stackView = UIStackView(frame: .zero)
        stackView.axis = .vertical
        stackView.alignment = .fill
        stackView.distribution = .equalSpacing
...
private extension KeyboardInputViewController {
    ...
    func addLongPressGesture(to button: KeyboardButton) {
        if button.action == .shift || button.action == .shiftDown{
            printLog("shift button")
            button.removeLongPressAction()
        }else{
